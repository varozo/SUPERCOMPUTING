...
- Overall, I learned the multiple ways (and their tradeoffs) for these methods to navigate creating and using environments to help me add tools specific to what I need. I also learned how to use Flye to “assemble” the genome data, which was also a big of a struggle (specifically navigating what to put for my Flye arguments).
- When I outputted the last 10 lines of each log file for each method, I noticed the outputs were the same, so I decided to cat the entire log outputs, which showed minor differences, which were insignificant. 
- But regarding the methods themselves, I thought that the conda method is the most intuitive for me since I apprieciated how I can add anything under my conda environment and it's the most reproducible of the three. For example, I cant download  ‘tree’ package because I don’t have permission to from admin, but I can use it if I download it in the environment. The only downside is remembering the commands to activate the environment since we aren’t downloading Flye like in the module load method, but they only happen to be two.
- I didn't completely favor the local build method since we have to manually change the version each time we choose to run it, but I made sure to add a note in the README. But apart from that, it only needed one line (add path) in its script to run so it has a fairly simple implementation.
- My least favorite method was the module load method. though it also only requires one line compared to the multiple for building the conda environment, this method relies on the HPC admins. We needed to see where Flye is in this case, so I needed to run `module avail flye` seperately to get it, which hinders reproducibility if its activation line is different per user. Another downside is that its limited to what the HPC admins have available (again, I wasnt able to directly download the ‘tree’ package on my HPC).
